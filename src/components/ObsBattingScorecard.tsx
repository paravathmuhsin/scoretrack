...
export function ObsBattingScorecard({ match, cfg, state, events }: Props) {
  ...
  const teamName = (
    battingSide === "home" ? match.home.name : match.away.name
  ).toUpperCase();
  ...
  const comp = inningsExtrasBreakdownFromBalls(
    events,
    inn.innings,
    battingSide,
  );
  ...
  return (
    <div className="obs-sc obs-sc--batting">
              ...
              <thead>
                <tr className="obs-sc-thead-row obs-sc-thead-row--bat">
                  <th
                    colSpan={2}
                    scope="colgroup"
                    className="obs-sc-head-meta obs-sc-th-meta"
                  >
                    {subtitle}
                  </th>
              ...
              <tbody>
                {squadBat.length === 0 ? (
                  ...
                  squadBat.map((p) => {
                    ...
                    const atCrease =
                      inn.strikerId === p.playerId ||
                      inn.nonStrikerId === p.playerId;
                    const status = overlayBattingStatusCell(
                      match,
                      bs,
                      inn,
                      p.playerId,
                    ...
                    return (
                      ...
                      >
                        <td className="obs-sc-name">
                          {nameFor(match, p.playerId).toUpperCase()}
                        </td>
