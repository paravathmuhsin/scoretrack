import { oversProgressString, type ReplayConfig, type ReplayState } from '../scoring/engine'
...
export function matchCompleteScoreLines(
  ...
  const lines: string[] = [
    `${firstName}: ${i1.runs}/${i1.wickets} (${oversProgressString(i1.legalBalls, cfg.ballsPerOver, cfg.oversLimit)} ov)`,
  ]
  if (state.innings2) {
    const i2 = state.innings2
    const secondName = i2.battingSide === 'home' ? match.home.name : match.away.name
    lines.push(
      `${secondName}: ${i2.runs}/${i2.wickets} (${oversProgressString(i2.legalBalls, cfg.ballsPerOver, cfg.oversLimit)} ov)`,
    )
  }
  return lines
}
